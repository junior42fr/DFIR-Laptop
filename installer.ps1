...
$global:SOURCE_dotPeek = "https://download.jetbrains.com/resharper/dotUltimate.2021.2.1/dotPeek64.2021.2.1.exe"
...
function dotPeek-Downloader{
    $ProgressPreference = 'SilentlyContinue'
    Write-Host "Téléchargement .Peek (71Mo)" -ForegroundColor DarkBlue -BackgroundColor White
    $dotpeek_version = $global:SOURCE_dotPeek.Split("/")[-1]
    $dotpeek_sauvegarde = $Chemin_forensic_tools + $dotpeek_version
    Invoke-WebRequest -Uri $global:SOURCE_dotPeek -UseBasicParsing -OutFile $dotpeek_sauvegarde
}
...
function WindowsOfflineToolsInstaller($installation){
    ...
    Foreach ($logiciel in $logiciels_msixbundle){
        Write-Host "Installation de " $logiciel.FullName -ForegroundColor DarkBlue -BackgroundColor White
        & Add-AppxPackage -Path $logiciel.FullName
    }
    ...
    #Installation de l'extension pack de VirtualBox si VBox est présent
    $virtualbox_install = $(@(Get-WmiObject -Class Win32_Product).Name) -match "virtualbox"
    if($virtualbox_install){
        $logiciels_vbox_extpack = Get-ChildItem -Recurse -Path $installation.chemin_logiciels -Include *.vbox-extpack | select FullName
        Foreach ($logiciel in $logiciels_vbox_extpack){
            Write-Host "Installation de " $logiciel.FullName -ForegroundColor DarkBlue -BackgroundColor White
