$global:SOURCE_VCRedist = "Microsoft.VCRedist.2015+.x86"
...
function VCRedist-Installer([string]$chemin_dl,[string]$chemin_log){
   	$logiciel = Get-ChildItem -Recurse -Path $chemin_dl -Include *Visual*2015-2022*x86*.exe | select FullName
	if($logiciel.FullName){
        Write-Host "Installation de " $logiciel.FullName -ForegroundColor DarkBlue -BackgroundColor White
		& $logiciel.FullName /passive /install
        Add-Content $chemin_log "Installation de VCRedist-x86 effectuee"
    }
    else{
        Write-Host "Aucun logiciel VCRedist-x86 a installer" -BackgroundColor White -ForegroundColor Red
        Add-Content $chemin_log "Aucun logiciel VCRedist-x86 a installer"
    }
	Add-Content $chemin_log '-------------------------'
}
